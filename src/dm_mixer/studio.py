...
class SoundbankStudioController(tk.Frame):
    def __init__(self, parent, on_config_changed_callback):
        """
        Tab 2 Dashboard: Handles importing raw audio tracks code-free 
        and maps them dynamically to a visual asset database table.
        """
        ...
        title = tk.Label(self, text="⚙️ IMPORT CUSTOM ATMOSPHERE ASSETS", fg="#ffcc00", bg="#1e1e1e", font=("Arial", 11, "bold"))
        ...
        save_btn = tk.Button(self, text="➕ Add Sound to Campaign Library", font=("Arial", 11, "bold"), bg="#5cb85c", fg="white", activebackground="#4cae4c", relief="flat", padx=10, pady=4, command=self.save)
    ...
    def save(self):
        ...
        try:
            ...
            new_kws = []
            for kw in raw_kws:
                new_kws.append(kw if self.loop_var.get() else f"!{kw}")
    ...
    def update_library_inventory_gui(self):
        ...
        try:
            ...
            for path, keywords in filtered_config.items():
                ...
                # Render the row record parameters straight into the database table
